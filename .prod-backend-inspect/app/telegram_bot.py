...
def _campaign_schedule_for_day(day_key: date, slot_labels: list[str]) -> list[CreativeCampaignTheme]:
    ...
    while len(scheduled) < len(slot_labels):
        ...
        random.Random(_stable_seed(day_key.isoformat(), f"batch-{batch_index}")).shuffle(batch)
...
def automated_daily_posts() -> list[DailyCommunityPost]:
    ref_day = date.today()
    return [build_automated_daily_post(ref_day, slot_key) for slot_key in auto_post_times_labels()]


def build_automated_daily_post(day_key: date | None = None, slot_key: str = "") -> DailyCommunityPost:
    ref_day = day_key or date.today()
    slot_labels = auto_post_times_labels()
    resolved_slot = str(slot_key or (slot_labels[0] if slot_labels else "08:00")).strip()
    if resolved_slot not in slot_labels:
        slot_labels = [resolved_slot] + [item for item in slot_labels if item != resolved_slot]
    ...
    if not scheduled:
        return DailyCommunityPost(
            ...
            text=_compact_lines(
                ...
                "Abra o app, fa̤a uma rodada curta e volta ao grupo com o seu feedback.",
