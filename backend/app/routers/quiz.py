...
def _raise_ai_provider_failure(action: str, provider: str, exc: Exception) -> None:
    ...
    if isinstance(exc, httpx.TimeoutException):
        raise HTTPException(
            status_code=504,
            detail=(
                f"O provedor {provider_label} demorou demais para {action}. "
                "O servidor esta online; tente novamente em instantes."
            ),
        )
    ...
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        detail = f"Erro no provedor {provider_label} ao {action} (HTTP {status})."
        if provider_detail:
            detail = f"{detail} Detalhe: {provider_detail}"
        raise HTTPException(status_code=502, detail=detail)
...
def generate_quiz(
    ...
    # ── Otimização de Nuvem: Checa Smart Question Cache se não houver contexto manual
    questions: list[dict] = []
    if not body.context and not body.document_id:
        try:
            cached_candidates = SmartQuestionCache.get_candidate_questions(
                db, topic=body.topic, difficulty=body.difficulty, limit=quantity * 2
            )
            ...
            if len(valid_cached) >= quantity:
                questions = valid_cached[:quantity]
                logger.info(
                    "quiz/generate cache hit topic='%s' count=%d",
                    body.topic, len(questions)
                )
        except Exception as exc:
...
class StudyPlanNoticeIn(BaseModel):
    job_title: str | None = Field(default=None, max_length=160)
    job_id: str | None = Field(default=None, max_length=60)
    job_education: str | None = Field(default=None, max_length=100)
    job_vacancies: int | None = Field(default=None, ge=0, le=100_000)
    notice_text: str = Field(min_length=20, max_length=350_000)
    provider: str | None = Field(default=None, max_length=30)


@router.post("/study-plan/analyze-notice")
def analyze_study_plan_notice(
    body: StudyPlanNoticeIn,
    authorization: str | None = Header(default=None, alias="Authorization"),
    db: Session = Depends(get_db),
):
    user = _require_user(authorization, db)
    requested_job_title = (body.job_title or "").strip()
    prompt = ai.build_notice_analysis_prompt(
        job_title=requested_job_title,
        job_id=(body.job_id or "").strip(),
        job_education=(body.job_education or "").strip(),
    ...
    try:
        raw_text, _provider = _call_ai_for_user(
            ...
            db,
            requested_provider=body.provider,
            system_prompt=ai._SYSTEM_NOTICE,
            user_prompt=prompt,
        )
        parsed = ai.extract_json_object(raw_text)
        analysis = ai.normalize_notice_analysis(
            parsed,
        ...
        logger.warning("study-plan/analyze-notice error: %s", exc)
        _raise_ai_provider_failure(
            "analisar o edital",
            normalize_provider(body.provider),
            exc,
        )

    if not analysis["disciplinas"] and requested_job_title:
        raise HTTPException(
            status_code=422,
            detail=(
                "Nao foi possivel localizar conteudo programatico para esse cargo "
                "no PDF. Confira o nome do cargo e o edital selecionado."
            ),
        )
    if not requested_job_title and not analysis["cargos_popup"]:
        raise HTTPException(
            status_code=422,
            detail="Nao foi possivel localizar cargos no edital.",
        )
    return analysis
