...
async def security_headers(request: Request, call_next):
    ...
    if request.url.path.startswith(
        ("/auth", "/admin", "/user", "/billing", "/internal")
    ):
...
def _ensure_user_settings_row(db: Session, user_id: int) -> models.UserSettings:
    row = (
        ...
        .filter(models.UserSettings.user_id == int(user_id))
        .first()
    )
    if row:
        return row
...
def _user_settings_out(
    row: models.UserSettings, user_id: int
...
@app.api_route("/app/download/android/{version}.apk", methods=["GET", "HEAD"])
...
def telegram_group_provision(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"telegram_provision_failed: {ex}"
        ) from ex
...
def telegram_webhook_configure(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"telegram_webhook_config_failed: {ex}"
        ) from ex
...
def upsert_user_settings(
    ...
    if payload.api_key_gemini is not None:
        row.api_key_gemini = services.encrypt_api_key(
            APP_SECRET, payload.api_key_gemini
        )
    if payload.api_key_openai is not None:
        row.api_key_openai = services.encrypt_api_key(
            APP_SECRET, payload.api_key_openai
        )
